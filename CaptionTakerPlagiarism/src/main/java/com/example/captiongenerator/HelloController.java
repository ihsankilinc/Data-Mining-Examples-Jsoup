...
public class HelloController {
    ...
    @FXML
    protected void onKontrolButtonClick(ActionEvent event) throws IOException {

        Connector.main();
        Connector.loadText("OzgunBasliklar.txt");

        percentageText.setText("Yüzde bin benzerlik.");
        String linksFileName = file.getAbsolutePath();

        Document doc = Jsoup.parse(new File(linksFileName), "utf-8");

        //array
        ArrayList<String> baslikdizisi = new ArrayList<String>();
        ArrayList<String> cikarilandizisi = new ArrayList<String>();
        ArrayList<String> linklerdizisi = new ArrayList<String>();
        ...
        Elements links = doc.select("td.text-center a.btn.btn-xs.btn-outline.btn-info");

        for (Element link:links) {
            System.out.println(link.text());

            if(link.attr("abs:href").contains("gundem")||link.attr("abs:href").contains("saglik")||link.attr("abs:href").contains("magazin")){
                System.out.println(link.attr("abs:href"));
            }
            else{
                linklerdizisi.add(link.attr("abs:href"));
            }

        }
//        link.attr("abs:href")+link.text()+

        ArrayList<String> allFatherText = new ArrayList<>();
        String s="";

        for(int i=0;i<linklerdizisi.size();i++){
            try {

                Document visitedLink = Jsoup.connect(linklerdizisi.get(i)).get();
                Elements basliklar = visitedLink.select("h2.lead");

                if(basliklar.text()==""){
                    continue;
                }
                else{
                    System.out.println(basliklar.text().toUpperCase(new Locale("tr","TR")).trim()+" *** ");
                    s+=basliklar.text().toUpperCase(new Locale("tr","TR")).trim()+" *** ";
                    allFatherText.add(basliklar.text().toUpperCase(new Locale("tr","TR")).trim()+" *** ");
                }
            }
        ...
        Path out = Paths.get("spotlar.txt");
        Files.write(out, Collections.singleton(s), Charset.defaultCharset());
    ...
    protected void onHelloButtonClick() throws IOException {
        ...
        while(min<=max){
            if(doc.getElementById("row-"+min)==null){
                ...
                if(arrGoruntulenme[arrGoruntulenme.length-4].matches(".*\\d.*") && arrGoruntulenme[arrGoruntulenme.length-4].contains("-")==false){
                    ...
                    maxHitOnly.add(Integer.valueOf(arrGoruntulenme[arrGoruntulenme.length-3]));
                    maxHitName.add(secondLevelChildElement.text());
                    System.out.println(sum);
                    sum+=Integer.valueOf(arrGoruntulenme[arrGoruntulenme.length-3]);
                }
